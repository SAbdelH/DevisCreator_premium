WITH achat_semaine AS (
    ...
    FROM (
        ...
        SELECT 
            EXTRACT(YEAR FROM (CURRENT_DATE - INTERVAL '3 weeks'))::integer AS annee,
            EXTRACT(WEEK FROM (CURRENT_DATE - INTERVAL '3 weeks'))::integer AS semaine_num,
            (date_trunc('week', CURRENT_DATE - INTERVAL '3 weeks'))::date AS semaine_debut,
            (date_trunc('week', CURRENT_DATE - INTERVAL '3 weeks') + INTERVAL '1 week - 1 day')::date AS semaine_fin
        UNION
        SELECT 
...
SELECT 
    'Semaine ' || a.semaine_num AS semaines, f.facture - a.achat "Bénéfice"
